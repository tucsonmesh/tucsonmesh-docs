...
The Install Coordinator will contact the Installee Member to set up a site survey. Instructions on how to perform a site survey can be found in the [site survey guide](../site-survey/index.md).

### 3. Install preparation

Before the install, the installation team will need to collect any additional information from the Installee Member, schedule the install date, make an install plan and configure the routers.

### 4. Install

On the day of the install, the team will mount and align the rooftop routers, run ethernet cable from the roof to inside the house, connect and configure the indoor router.
...
Be sure to check the site survey notes in the Trello card. If any required information is missing, be sure to get it from the people who did the survey, or from the installee member, before scheduling the install.
...
Configure the outdoor routers before the day of the install. Instructions are available in the [Configuring Routers Guide](../configuring-routers/index.md)
...
### Make a patch cable to connect the rooftop routers
...
Go over the install plan, especially the locations where you plan to drill into the structure, and where you plan to place the indoor router. Make sure nothing has changed.

### Check the rooftop safety

Before going up on the roof, ask the Installee Member to point out any hazards that are new since the site survey.

Note any hazards identified during the site survey and point them out to the team. These could include things like trip and slip hazards, loose cables and unprotected ledges.
...
1. Select an appropriately-sized hose clamp. 
2. Thread the hose clamp through the router mount.
3. Tighten the hose clamp using a screwdriver or drill on screw setting.
...
### Run cable from the roof to inside the house
...
Confirm that you can connect to the inside router using the SSID and password provide by the Installee Member.
...
Move the card to the `Installed` list.
